            ...
            [ K.Container::{
              , env = Some
                [ 
                  ...
                  , K.EnvVar::{name = "RPCBIND", value = Some ":80"} 
                  , K.EnvVar::{name = "RPCPASSWORD", value = Some "developer"} 
                  , K.EnvVar::{name = "RPCPORT", value = Some "80"} 
                  ...
                  , K.EnvVar::{name = "ZMQPUBRAWBLOCK", value = Some "tcp://0.0.0.0:39703"} 
                  , K.EnvVar::{name = "ZMQPUBRAWTX", value = Some "tcp://0.0.0.0:39704"} 
                ]
              , name = name
              , image = Some "heathmont/bitcoind:v1.0.9"
              , ports = Some
                [ K.ContainerPort::{ containerPort = 39703 }, K.ContainerPort::{ containerPort = 39704 } ]
              , volumeMounts = Some [ K.VolumeMount::{ mountPath = "/bitcoin/.bitcoin", name = name } ]
              }
            ]
          , hostname = Some name
