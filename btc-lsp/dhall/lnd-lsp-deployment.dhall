        ...
        , spec = Some K.PodSpec::{
          , containers =
            [ K.Container::{
              ...
              , args = Some ["-c", "lnd --bitcoin.active --bitcoin.\$\$BITCOIN_NETWORK --bitcoin.node=bitcoind --bitcoin.defaultchanconfs=\$\$BITCOIN_DEFAULTCHANCONFS --bitcoind.rpchost=\$\$BITCOIN_RPCHOST --bitcoind.rpcuser=\$\$BITCOIN_RPCUSER --bitcoind.rpcpass=\$\$BITCOIN_RPCPASS --bitcoind.zmqpubrawblock=\$\$BITCOIN_ZMQPUBRAWBLOCK --bitcoind.zmqpubrawtx=\$\$BITCOIN_ZMQPUBRAWTX --tlsextradomain=\$\$TLS_EXTRADOMAIN --restlisten=0.0.0.0:\$\$LND_REST_PORT --rpclisten=0.0.0.0:\$\$LND_GRPC_PORT --listen=0.0.0.0:\$\$LND_P2P_PORT --maxpendingchannels=100"]
              , command = Some ["sh"]
              , env = Some
                [ 
                  K.EnvVar::{name = "BITCOIN_DEFAULTCHANCONFS", value = Some "1"} 
                  , K.EnvVar::{name = "BITCOIN_NETWORK", value = Some "regtest"} 
                  , K.EnvVar::{name = "BITCOIN_RPCHOST", value = Some "bitcoind:80"} 
                  , K.EnvVar::{name = "BITCOIN_RPCPASS", value = Some "developer"} 
                  , K.EnvVar::{name = "BITCOIN_RPCUSER", value = Some "bitcoinrpc"} 
                  , K.EnvVar::{name = "BITCOIN_ZMQPUBRAWBLOCK", value = Some "tcp://bitcoind:39703"} 
                  , K.EnvVar::{name = "BITCOIN_ZMQPUBRAWTX", value = Some "tcp://bitcoind:39704"} 
                  ...
                  , K.EnvVar::{name = "TLS_EXTRADOMAIN", value = Some "lnd-lsp"} 
                ]
              , ports = Some
                [ K.ContainerPort::{ containerPort = 8080 }, K.ContainerPort::{ containerPort = 9735 }, K.ContainerPort::{ containerPort = 10009 } ]
              , volumeMounts = Some [ K.VolumeMount::{ mountPath = "/root/.lnd", name = name } ]
              }
            ]
          , hostname = Some name
